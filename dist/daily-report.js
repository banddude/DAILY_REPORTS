...
function ensureDir(dirPath) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            yield fs.promises.access(dirPath);
        }
        catch (error) {
...
function extractFramesFromData(videoPath, frameDataPath, outputDir) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const jsonData = yield fs.promises.readFile(frameDataPath, 'utf-8');
...
function convertVideoToAudio(videoPath, audioPath) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            yield fs.promises.access(audioPath);
...
function transcribeAudio(audioPath, profileData) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const transcription = yield openai.audio.transcriptions.create({
            file: yield (0, uploads_1.toFile)(fs.createReadStream(audioPath), path_1.default.basename(audioPath)), // Use fs.createReadStream and path.basename
...
function generatePdfReport(reportDataPath, imagesDir, outputPdfPath, logoFilename) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a, _b, _c, _d, _e;
        try {
            const reportJsonString = yield fs.promises.readFile(reportDataPath, 'utf-8');
            ...
            const doc = new pdfkit_1.default({ margin: 50, layout: 'portrait', size: 'A4' });
            const writeStream = fs.createWriteStream(outputPdfPath);
            doc.pipe(writeStream);
            // --- Header --- 
            const logoPathInReportDir = path_1.default.join(path_1.default.dirname(outputPdfPath), logoFilename);
            let logoExists = false;
            try {
                yield fs.promises.access(logoPathInReportDir);
                logoExists = true;
            }
            catch (_f) { }
            if (logoExists) {
            ...
            let infoBlock = '';
            if (companyInfo.name)
                infoBlock += `${companyInfo.name.trim()}\n`;
            if (companyAddress) { // Use companyAddress extracted from reportData
                if (companyAddress.street)
                    infoBlock += `${companyAddress.street}${companyAddress.unit ? ' #' + companyAddress.unit : ''}\n`;
                if (companyAddress.city || companyAddress.state || companyAddress.zip)
                    infoBlock += `${companyAddress.city || ''}${companyAddress.city && companyAddress.state ? ', ' : ''}${companyAddress.state || ''} ${companyAddress.zip || ''}\n`;
            }
            let contactLine = '';
            if (companyInfo.phone)
                contactLine += `Phone: ${companyInfo.phone}`;
            if (companyInfo.website)
                contactLine += `${contactLine ? ' | ' : ''}Website: ${companyInfo.website}`;
            if (contactLine)
                infoBlock += contactLine;
            ...
            // Prepared By / Date
            let preparedLine = '';
            if (preparedByInfo.name)
                preparedLine += `Prepared By: ${preparedByInfo.name}${preparedByInfo.email ? ' (' + preparedByInfo.email + ')' : ''}`; // Use preparedByInfo
            const reportDirName = path_1.default.basename(path_1.default.dirname(outputPdfPath));
            ...
            doc.moveDown(0.3);
            if (((_a = reportData.workCompleted) === null || _a === void 0 ? void 0 : _a.length) > 0) {
            ...
            doc.moveDown(0.3);
            if (((_b = reportData.issues) === null || _b === void 0 ? void 0 : _b.length) > 0) {
                reportData.issues.forEach((item) => {
                    doc.font('Helvetica').fontSize(10).text(`• [${item.status || 'Unknown'}] ${item.description || 'N/A'}`);
                    if (item.impact)
                        doc.font('Helvetica-Oblique').fontSize(9).text(` Impact: ${item.impact}`, { indent: 20 });
                    if (item.resolution)
                        doc.font('Helvetica-Oblique').fontSize(9).text(` Resolution: ${item.resolution}`, { indent: 20 });
                    doc.moveDown(0.2);
                });
            }
            else {
            ...
            doc.moveDown(0.3);
            if (((_c = reportData.materials) === null || _c === void 0 ? void 0 : _c.length) > 0) {
            ...
            doc.moveDown(0.3);
            if (((_d = reportData.nextSteps) === null || _d === void 0 ? void 0 : _d.length) > 0) {
            ...
            if (((_e = reportData.images) === null || _e === void 0 ? void 0 : _e.length) > 0) {
                doc.addPage();
                ...
                if (imageMaxHeight < 100)
                    imageMaxHeight = 100; // Set a minimum sensible height
                if (imageMaxHeight > 280)
                    imageMaxHeight = 280; // Set a maximum sensible height
                ...
                for (let i = 0; i < reportData.images.length; i++) {
                    ...
                    const imagePath = path_1.default.join(imagesDir, imageInfo.fileName);
                    ...
                    try {
                        yield fs.promises.access(imagePath);
...
function generateReport(inputVideoPath) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a, _b, _c, _d, _e, _f;
        console.log(`Starting report generation for video: ${inputVideoPath}`);
        // --- Profile and Logo Setup ---
        let profileData = {};
        const profilePath = path_1.default.join(__dirname, 'profile.json'); // Use __dirname
        // Read profile data first
        try {
            const profileJsonString = yield fs.promises.readFile(profilePath, 'utf-8');
        ...
        const logoSourcePath = path_1.default.join(__dirname, logoSourceFilename); // Use __dirname
        // --- Timestamped Directory Setup ---
        const now = new Date();
        const timestamp = now.toISOString().replace(/[:.]/g, '-');
        const outputBaseDir = path_1.default.join('.', `report_${timestamp}`);
        yield ensureDir(outputBaseDir);
        console.log(`Outputting files to: ${outputBaseDir}`);
        // --- Copy Logo to Output Directory ---
        const logoDestPath = path_1.default.join(outputBaseDir, logoSourceFilename);
        try {
            yield fs.promises.copyFile(logoSourcePath, logoDestPath);
        ...
        const videoDestPath = path_1.default.join(outputBaseDir, path_1.default.basename(videoPath));
        try {
            yield fs.promises.copyFile(videoPath, videoDestPath);
        ...
        const audioPath = path_1.default.join(outputBaseDir, 'output.mp3');
        const transcriptDataPath = path_1.default.join(outputBaseDir, 'transcript_data.json');
        const reportJsonPath = path_1.default.join(outputBaseDir, 'daily_report.json');
        const framesDirPath = path_1.default.join(outputBaseDir, 'extracted_frames'); // Frames subdir
        const pdfOutputPath = path_1.default.join(outputBaseDir, 'daily_report.pdf');
        const viewerDestPath = path_1.default.join(outputBaseDir, 'report-viewer.html'); // Define viewer path
        const viewerSourcePath = path_1.default.join(__dirname, 'report-viewer.html'); // Add source path for viewer
        let transcriptionResult = null;
        let reportJson = null; // To hold the report object
        let finalReportViewerUrl = null; // Variable to store the final URL
        try {
            // 1. Convert video to audio (using updated audioPath)
            yield convertVideoToAudio(videoPath, audioPath);
            // 2. Transcribe audio (using updated audioPath)
            transcriptionResult = yield transcribeAudio(audioPath, profileData);
            ...
            yield fs.promises.writeFile(transcriptDataPath, JSON.stringify(transcriptionResult, null, 2));
            ...
            reportJson = yield getDailyReport(transcriptionResult, profileData);
            if (!reportJson) {
            ...
            // --- Delete Audio File ---
            try {
                yield fs.promises.unlink(audioPath);
            ...
            try {
                yield fs.promises.rm(outputBaseDir, { recursive: true, force: true });
        ...
        try {
            yield ensureDir(framesDirPath); // Ensure frames subdir exists
            ...
            if (reportJson.images && Array.isArray(reportJson.images)) {
                ...
                for (let i = 0; i < reportJson.images.length; i++) {
                    ...
                    frameExtractionPromises.push(extractFrame(videoPath, timestamp, framesDirPath, fileName));
                    reportJson.images[i] = { fileName, caption };
                }
                yield Promise.all(frameExtractionPromises);
            ...
            reportJson.customer = (_a = profileData.company) === null || _a === void 0 ? void 0 : _a.customer;
            reportJson.project = (_b = profileData.company) === null || _b === void 0 ? void 0 : _b.project;
            ...
            const reportDirName = path_1.default.basename(outputBaseDir);
            let baseS3Url = null;
            let s3TargetPath = null;
            if (s3Bucket) {
            ...
            reportJson.reportMetadata = {
                ...
                companyInfo: {
                    name: ((_c = profileData.company) === null || _c === void 0 ? void 0 : _c.name) || null,
                    address: ((_d = profileData.company) === null || _d === void 0 ? void 0 : _d.address) || null,
                    phone: ((_e = profileData.company) === null || _e === void 0 ? void 0 : _e.phone) || null,
                    website: ((_f = profileData.company) === null || _f === void 0 ? void 0 : _f.website) || null,
                }
            };
            // ----------------------------------------------------
            ...
            reportJson.reportAssetsS3Urls = {
                baseUrl: baseS3Url,
                videoUrl: baseS3Url ? `${baseS3Url}${path_1.default.basename(videoDestPath)}` : null,
                transcriptJsonUrl: baseS3Url ? `${baseS3Url}${path_1.default.basename(transcriptDataPath)}` : null,
                pdfUrl: baseS3Url ? `${baseS3Url}${path_1.default.basename(pdfOutputPath)}` : null,
                logoUrl: baseS3Url ? `${baseS3Url}${path_1.default.basename(logoDestPath)}` : null
            };
            if (reportJson.images && Array.isArray(reportJson.images)) {
                reportJson.images.forEach((imageInfo) => {
                    if (imageInfo.fileName) {
                        imageInfo.s3Url = baseS3Url ? `${baseS3Url}${path_1.default.basename(framesDirPath)}/${imageInfo.fileName}` : null;
                    }
                    else {
            ...
            yield fs.promises.writeFile(reportJsonPath, JSON.stringify(reportJson, null, 2));
            console.log(`Final daily report with metadata and S3 URLs saved to ${reportJsonPath}`);
            // ----------------------------------      
            // 5. Generate PDF Report
            console.log('\nGenerating PDF report...');
            yield generatePdfReport(reportJsonPath, framesDirPath, pdfOutputPath, logoSourceFilename);
            // 6. Copy Viewer HTML to Output Directory (ADD THIS STEP)
            try {
                yield fs.promises.copyFile(viewerSourcePath, viewerDestPath);
            ...
            if (s3Bucket && s3TargetPath) { // Check again (belt and suspenders)
                ...
                try { // Wrap entire upload/cleanup logic to ensure cleanup happens on error
                    if (!awsAccessKeyId || !awsSecretAccessKey) {
                        ...
                        yield execAsync(uploadCommand); // Throws on error
                        uploadSuccess = true;
                    }
                    else {
                        ...
                        yield execAsync(stsCommand); // Verify credentials
                    ...
                    yield fs.promises.rm(localSourcePath, { recursive: true, force: true });
            ...
            const reportJsonKey = `${customerFolder}/${projectFolder}/${reportDirName}/${path_1.default.basename(reportJsonPath)}`;
