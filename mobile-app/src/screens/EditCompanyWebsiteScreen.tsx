...
interface ProfileCompany { website?: string; }
...
function EditCompanyWebsiteScreen(): React.ReactElement {
  ...
  const fetchCurrentValue = useCallback(async () => {
    ...
    try {
      ...
      const value = data?.company?.website || '';
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const payload = { ...fullProfile, company: { ...(fullProfile.company || {}), website: currentValue } };
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['left', 'right', 'bottom']}>
      <ScrollView style={styles.container}>
        ...
        <TextInput style={styles.textInput} value={currentValue} onChangeText={setCurrentValue} placeholder="https://www.example.com" editable={!isSaving} keyboardType="url" autoCapitalize='none' />
      </ScrollView>
    </SafeAreaView>
  );
}
